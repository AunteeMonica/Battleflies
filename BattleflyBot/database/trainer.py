...
class TrainerDAO(DataDAO):
    ...
    def get_battlefly_inventory(self, user_id: str) -> dict:
        """
        Gets the inventory of battlefly from the trainer
        """
        return self.data[user_id][self.PINVENTORY]

    def get_battlefly_quantity(self, user_id: str, pkmn_name: str) -> int:
        """
    ...
    def increment_battlefly_quantity(self, user_id: str, pkmn_name: str) -> None:
        """
        Increments the quantity of a specific trainer's battlefly
        within their inventory
        """
        pinventory = self.get_battlefly_inventory(user_id)
        if pkmn_name not in pinventory:
            self.set_battlefly_quantity(user_id, pkmn_name, 1)
        else:
            self.data[user_id][self.PINVENTORY][pkmn_name] += 1

    def set_battlefly_quantity(self, user_id: str, pkmn_name: str, quantity: int) -> None:
        """
        Sets the quantity of a specific trainer's battlefly
        within their inventory
        """
        if quantity <= 0:
    ...
    def get_total_battlefly_caught(self) -> int:
        """
        Gets the total amount of battlefly caught across
        all trainers
        """
        total_battlefly_caught = 0
        for trainer in self.data:
            total_battlefly_caught += self.data[trainer][self.TOTAL_PKMN_COUNT]
        return total_battlefly_caught
    ...
    def increment_legendary_pkmn_count(self, user_id: str) -> None:
        """
        Increments the trainer's count for legendary battlefly
        """
    ...
    def decrease_total_pkmn_count(
        ...
        """
        Decreases the total battlefly count by a specified amount
        """
