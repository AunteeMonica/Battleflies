...
class battleflyBotCog(commands.Cog):
    ...
    async def post_battlefly_does_not_exist_exception_msg(
        self,
        ctx: commands.Context,
        e: Exception
    ) -> None:
        """
        Sends the exception message for when a given battlefly
        does not exist
        """
        await ctx.send(f"{ctx.message.author.mention},"
                       f" the battlefly '**{str(e)}**' does not"
                       " exist. Please specify a valid battlefly name.")
    ...
    async def post_higher_quantity_specified_exception_msg(
        ...
        specified over the max quantity of a battlefly to release
        """
        await ctx.send(f"{ctx.message.author.mention},"
                       " please specify a battlefly quantity less than or"
                       f" equal to the max battlefly quantity number:"
                       f" **{str(e)}**")
    ...
    async def post_not_enough_exchange_battlefly_quantity_exception_msg(
        self,
        ctx: commands.Context
    ) -> None:
        """
        Sends the exception message for when not enough battlefly
        exists to exchange
        """
        await ctx.send(f"{ctx.message.author.mention},"
                       " please specify a valid amount of"
                       " battlefly to exchange that have enough"
                       " quantity to release")

    async def post_not_enough_exchange_battlefly_specified_exception(
        self,
        ctx: commands.Context
    ) -> None:
        """
        ...
        """
        await ctx.send(f"{ctx.message.author.mention},"
                       " please specify five battlefly to exchange")
    ...
    async def post_release_quantity_too_low_msg(
        ...
        """
        await ctx.send(f"{ctx.message.author.mention},"
                       " please specify a battlefly quantity greater than 0"
                       " to release")

    async def post_too_many_exchange_battlefly_specified_exception_msg(
        self,
        ctx: commands.Context
    ) -> None:
        """
        Sends the exception message for when more than the minimum number
        of battlefly is specified to exchange
        """
        await ctx.send(f"{ctx.message.author.mention},"
                       " please specify only 5 battlefly to exchange")

    async def post_unavailable_battlefly_to_trade_exception_msg(
        self,
        ctx: commands.Context,
        e: Exception
    ) -> None:
        ...
        battlefly required to trade the night vendor
        """
        await ctx.send(f"{ctx.message.author.mention},"
    ...
    async def post_unregistered_trainer_exception_msg(
        ...
        await ctx.send(f"{ctx.message.author.mention},"
                       ...
                       " a battlefly first in order to use this"
                       " bot command)")
