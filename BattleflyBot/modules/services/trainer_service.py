...
class TrainerService(battleflyBotModule):
    ...
    def give_battlefly_to_trainer(self, user_id: str, pkmn: battlefly) -> None:
        """
        ...
        try:
            ...
            if pkmn.is_egg:
                ...
                self.trainer_dao.increment_battlefly_quantity(user_id, pkmn_name)
    ...
    def get_all_total_battlefly_caught_count(self) -> int:
        """
        Gets the total of all battlefly caught
        """
        try:
            return self.trainer_dao.get_total_battlefly_caught()
        except Exception as e:
            msg = "Error has occurred in getting total battlefly caught."
            self.post_error_log_msg(TrainerServiceException.__name__, msg, e)
            raise

    def get_trainer_total_battlefly_caught(self, user_id: str) -> int:
        """
        Gets the trainer's total battlefly caught
        """
        try:
            return self.trainer_dao.get_total_pkmn_count(user_id)
        except Exception as e:
            msg = "Error has occurred in getting trainer total battlefly."
            self.post_error_log_msg(TrainerServiceException.__name__, msg, e)
            raise
    ...
    async def get_trainer_inventory(self, user_id: str) -> dict:
        ...
        """
        try:
            return self.trainer_dao.get_battlefly_inventory(user_id)
        except Exception as e:
            msg = "Error has occurred in getting trainer inventory."
            self.post_error_log_msg(TrainerServiceException.__name__, msg, e)
            raise

    async def decrease_battlefly_quantity(
        self,
        user_id: str,
        pkmn: battlefly,
        quantity: int
        ...
        try:
            ...
            if pkmn.is_shiny:
                pkmn_name = "(shiny)" + pkmn_name
            battlefly_quantity = \
                self.trainer_dao.get_battlefly_quantity(user_id, pkmn_name)
            if quantity > battlefly_quantity:
                raise HigherReleaseQuantitySpecifiedException(battlefly_quantity)
            new_battlefly_quantity = battlefly_quantity - quantity
            self.trainer_dao.set_battlefly_quantity(
                user_id,
                pkmn_name,
                new_battlefly_quantity
            )
            if pkmn.is_legendary:
            ...
            msg = "Error has occurred in deleting battlefly."
            self.post_error_log_msg(TrainerServiceException.__name__, msg, e)
            raise
    ...
    def get_quantity_of_specified_battlefly(self, user_id: str, pkmn_name: str):
        """
        Gets the quantity of a specified battlefly that the trainer has
        """
        try:
            return self.trainer_dao.get_battlefly_quantity(user_id, pkmn_name)
        except Exception as e:
